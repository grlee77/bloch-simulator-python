...
def process_gradient_argument(gr, points):
    ...
    If only one number is passed, it's assigned to the entire xgrad and the
    others are zeros.
    """
...
def process_off_resonance_arguments(df):
    ...
    Returns df and size. If only one numer is passed, returns number as single
    array.
    """
...
def process_positions(dp):
    ...
    if 3 == position_dimensions:
        ...
        return dp[0, :], dp[1, :], np.zeros(
            number_of_positions), number_of_positions
    else:
        return dp[0, :], np.zeros(number_of_positions), np.zeros(
            number_of_positions), number_of_positions
...
def process_magnetization(mx_0, my_0, mz_0, rf_length, freq_pos_count, mode):
    ...
    """
    if isinstance(mx_0, np.ndarray) and isinstance(
            my_0, np.ndarray) and isinstance(mz_0, np.ndarray):
        mx_0 = mx_0.ravel()
        my_0 = my_0.ravel()
        mz_0 = mz_0.ravel()
    out_points = 1
    ...
    if mx_0 is not None and not isinstance(mx_0, type(0.0)) and \
        not isinstance(my_0, type(0)) and freq_pos_count == mx_0.size and \
            freq_pos_count == my_0.size and freq_pos_count == mz_0.size:
